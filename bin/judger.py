...
def test_judge_imp(judge):
	...
	try:
		...
		if judge.language == 'c++':
			...
			compile_command = 'g++ %s -o %s -ansi -fno-asm -O2 -Wall -lm --static -DONLINE_JUDGE 2>%s' % (source_filename, exe_filename, log_filename)
			...
			exe_filename = base_filename + '.exe'
			compile_command = 'gcc %s -o %s -ansi -fno-asm -O2 -Wall -lm --static -DONLINE_JUDGE 2>%s' % (source_filename, exe_filename, log_filename)
			if(os.system(compile_command)):
				logfile = open(log_filename)
				set_judge_result(judge, 'CE', logfile.read())
				logfile.close()
...
#		if not os.path.exists(exe_filename):  #added by ymc
#			logfile = open(log_filename)
#			set_judge_result(judge, 'CE',logfile.read())
#			logfile.close()
#			return
...
#		time.sleep(3)  #added by ymc
		...
		for testcase in oj.problem.models.ProblemTestData.objects.filter(problem__exact = judge.problem):
			...
			popen=subprocess.Popen(exe_filename,stdin=input_file.fileno(),stdout=output_file.fileno())
			print popen.pid
			input_file.close()	#added by ymc
			output_file.close()	#added by ymc
			...
			while 1:
				try:
					statfile = open('/proc/%d/stat'%popen.pid, 'r')
				...
				newpid, status = os.waitpid(popen.pid, os.WNOHANG)
				if newpid != 0:
					break
	...
	#	output_file.close()
		...
		userprofile=judge.user.get_profile()
		userprofile.save()
		...
		judge.problem.save()
		if judge.incontest:
			contest=judge.incontest
			contestuser=oj.userprofile.models.ContestUser.objects.get(userprofile__exact=userprofile.id,contest__exact=contest.id)
			contestuser.accept_counts += 1
			if not judge.problem in contestuser.accept_problems.all():
				contestuser.accept_problems.add(judge.problem)
				contestuser.accept_problems_counts +=1
			contestuser.save()
	except Exception,e:
...
def init_django():
	...
	import oj.userprofile.models
	import oj.contest.models
